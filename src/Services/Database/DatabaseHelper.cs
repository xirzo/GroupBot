...
public class DatabaseHelper
{
    ...
    private async Task Initialize()
    {
        ...
        await using (var command = new SQLiteCommand(createAdminsTableQuery, connection))
        {
            await command.ExecuteNonQueryAsync();
        }
    ...
    public async Task<List<Participant>> GetAllAdmins()
    {
        ...
        const string selectAdminsQuery =
            "SELECT u.telegram_id, u.full_name FROM admins a JOIN users u ON a.user_id = u.id;";

        await using var command = new SQLiteCommand(selectAdminsQuery, connection);
        await using var reader = await command.ExecuteReaderAsync();

        var admins = new List<Participant>();
